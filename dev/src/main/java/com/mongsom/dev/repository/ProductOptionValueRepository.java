...
public interface ProductOptionValueRepository extends JpaRepository<ProductOptionValue, Integer> {
    ...
    @Query("SELECT pov FROM ProductOptionValue pov WHERE pov.optionTypeId = :optionTypeId ORDER BY pov.sortOrder, pov.optionValueId")
    List<ProductOptionValue> findByOptionTypeIdOrderBySortOrder(@Param("optionTypeId") Integer optionTypeId);
    
    // 재고가 있는 옵션 값들만 조회
    @Query("SELECT pov FROM ProductOptionValue pov WHERE pov.optionTypeId = :optionTypeId AND pov.stockStatus = 1 ORDER BY pov.sortOrder")
