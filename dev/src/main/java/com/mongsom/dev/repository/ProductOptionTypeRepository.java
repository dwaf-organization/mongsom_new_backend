...
public interface ProductOptionTypeRepository extends JpaRepository<ProductOptionType, Integer> {
    ...
    @Query("SELECT pot FROM ProductOptionType pot WHERE pot.productId = :productId ORDER BY pot.sortOrder, pot.optionTypeId")
    List<ProductOptionType> findByProductIdOrderBySortOrder(@Param("productId") Integer productId);
    
    // 상품의 필수 옵션 타입들 조회
    @Query("SELECT pot FROM ProductOptionType pot WHERE pot.productId = :productId AND pot.isRequired = 1 ORDER BY pot.sortOrder")
    ...
    @Query("SELECT DISTINCT pot FROM ProductOptionType pot " +
           "LEFT JOIN FETCH pot.optionValues " +
           "WHERE pot.productId = :productId " +
